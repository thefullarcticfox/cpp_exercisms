...
template<typename T>
static void test_sort_back(const tree_ptr<T> &tree, const std::vector<T> &expected)
{
    std::vector<T> actual;
...
TEST_CASE("can_sort_complex_tree_backwards")
{
    test_sort_back(make_tree<uint32_t>({2, 1, 3, 6, 7, 5}), {7, 6, 5, 3, 2, 1});
}


TEST_CASE("can_sort_complex_tree_strings_backwards")
{
    test_sort_back(make_tree<std::string>({"A", "few", "random", "strings", "that", "should", "be", "sorted"}),
        {"that", "strings", "sorted", "should", "random", "few", "be", "A"});
}
